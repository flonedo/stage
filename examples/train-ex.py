...
def get_dataset(dataset, args):
    if dataset.lower() == 'custom':
        lst_dataset = LstDetection('val.lst', root=os.path.expanduser('.'), flag=1, coord_normalized=False)
        train_dataset = lst_dataset
        val_dataset = lst_dataset
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers):
    ...
    train_loader = gluon.data.DataLoader(train_dataset, batch_size, True, last_batch='rollover', num_workers=num_workers)

    val_batchify_fn = Tuple(Stack(), Pad(pad_val=-1))
    '''val_loader = gluon.data.DataLoader(
        val_dataset.transform(YOLO3DefaultValTransform(width, height)),
        batch_size, False, batchify_fn=val_batchify_fn, last_batch='keep', num_workers=num_workers)'''
    val_loader = gluon.data.DataLoader(val_dataset, batch_size, True, last_batch='rollover', num_workers=num_workers)
...
# we dont't have evaluation metrics eval_metric (yet?)
def train(net, train_data, val_data, ctx, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            gt_boxes = gluon.utils.split_and_load(batch[6], ctx_list=ctx, batch_axis=0) # 6
            sum_losses = []
            obj_losses = []
            center_losses = []
            scale_losses = []
...
if __name__ == '__main__':
    ...
    # training data
    # we don't have evaluation metrics eval_metric(yet?)
